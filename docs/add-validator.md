...
> **注册没走完时，第 2 条会红**（2026-09-25 起）：
>
> ```
> [FAIL] validator  1 个声明的 L1 验证者**还不是链上成员**（l1-9）——
>                   它们在跑却没进集合，注册没走完：把 ACP-77 四步续完
> ```
>
> 在此之前它对这种状态判 `[OK] / READY / 0 failed` —— 一个人走完 ①–⑦ 后跑了这两条，
> 看见全绿就认为做完了，而那时链上成员还是 8。现在"活儿没干完"会红，
> 而"已被移除、只剩声明要清理"仍然不算故障（那两件事已经分开判）。
...
| `devnet-verify` 报 `validator … 还不是链上成员` | 注册没走完，回到 ⑨ 续完四步 | ⑨ |
