...
|---|---|---|---|---|---|
| `local` | linux | 127.0.0.1 | l1-1、l1-2、l1-3、l1-4、l1-5、primary-1、primary-2、l1-6 | 6 | host:single-machine |

容错：6 个等权验证者，查询门槛 75% → 可容忍 1 个离线。单边界形态，不做整机失效容错承诺。

#### `lan`（**当前生效**）

5 台独立物理机，5 个故障边界，每边界恰好 1 个 L1 验证者。2026-09-08 换过硬件并重新清点：五台的 MAC 互不相同、无虚拟机厂商 OUI、win-1 上已无 vmware-vmx 进程 —— 此前（2026-09-07）声明的 5 个边界里有 3 个是 win-1/win-2 上的 VMware 客户机，那次的 hypervisor 因素已随硬件更换消除。ubuntu-1/ubuntu-2 的 MAC OUI 为 DC:A6:32（Raspberry Pi Trading），即 arm64 —— 节点镜像走 TARGETARCH，binaries.env 备有 arm64 的 sha256。2 个 Primary 节点放在 Ubuntu 机器上：它们是验证者的引导目标，而 Windows 上的容器运行时需要用户登录才启动（ADR-0006）。清点方法与维护责任见 docs/adr/0007-failure-domain-independence.md。
...
容错：6 个等权验证者，查询门槛 75% → 可容忍 1 个离线。每边界至多 1 个验证者 → **可**容忍 1 个边界整体失效。
