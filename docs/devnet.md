...
| 顺序 | 先从集合移除 → 等确认 → **再停进程** | 直接从集合移除，没有进程可停 |
| 收尾 | 停进程 → 从 `deployment.json` 移除 → 重新渲染 | **先**从 `deployment.json` 移除并重新渲染 → **再**让那台机器开机 |

**走哪条由机器的实际状态决定，不由参数决定。** 工具会探测目标节点，与参数不符时停下来说清楚：

- 机器已失联却不加 `--emergency` → 拦下。走完四步后它会让你"去停进程"，而那台机器上没有进程可停；更要紧的是下面两个代价没被说出来。
- 机器还活着却加了 `--emergency` → 拦下。紧急路径跳过停进程，会留下一个**不在集合里却仍在运行**的节点：它不带权重、不破坏共识，但会一直占一行，而且机器重启后 `devnet-start` 还会把它拉起来。

**紧急摘除的两个代价**（其中第一个不明显）：

1. **少一个签名者。** 第二步要收集 L1 验证者的签名，而失联那台签不了。
   n=6、等权、门槛 67% ⇒ 需要 402/600，即至少 5 个 —— 死掉一个之后
   剩下 5 个**必须全签，一个都不能出问题**。而 §5.3 那类 P2P 签名故障
   （节点自己能签、别人经 P2P 要不到）说明"全签"不是理所当然的。
   工具在**动链之前**算这一笔，凑不够就拦下 —— 否则成员会卡在
   `pending-removed`，那时要靠重发消息才能往下走。
2. **无法确认它不会回来。** 优雅退出里进程是你自己停的，所以你知道它停了。
   紧急摘除时那台机器随时可能重新上线，而它的密钥仍在本机 ——
   所以收尾顺序反过来：先改声明、再开机。
...
> **这张表是"两个各 50%"这一个形态的结果，不是通用规律。**
> 决定成败的是**连上的权益比例 ≥ 80%**，不是"在线几个"。005 之后面板按
> 实测的权益分布判（`platform.getCurrentValidators` 不带 subnetID），
> 因此它对任意分布都给得出答案：
>
> | 权益分布 | 掉 1 个后 | 能否加入 |
> |---|---|---|
...
> | 6 × 16.7% | 83.3% | ✅ |
> | 8 × 12.5% | 87.5% | ✅ |
> | 2 × 49.7% + 6 × 0.09%（最小质押） | 50.3% | ❌ **加了 6 个却等于没加** |
>
> 最后一行是 005 取数时发现的：`minValidatorStake` 只有 2000 AVAX，而两个 Primary
> 各握 100 万 —— **按最小质押加验证者买到的是零提升**
> （见 [005 research R-07a](../specs/005-elastic-membership/research.md)）。
>
> **不要把拓扑设计在"恰好 80%"那一格上**：实测结论是"要求连上 ≥ 80%"，
> 而 avalanchego 实现取 `>` 还是 `>=` 我们没有直接证据，而失败形态是节点起不来。
>
> 读不到权益分布时，面板回落到"在服务的 Primary 数 < 2 即不行"，
> 也就是这张表 —— 那个回落值由 `tests/unit/recovery-stake-based.test.mjs`
> 与声明的 Primary 数比对，**改了 Primary 数就会变红**。
...
把"链现在什么状态"从**人主动跑一次命令得到的快照**，变成常驻、自动刷新的视图。
架构决策与四条"为何不"见 [ADR-0010](adr/0010-readonly-observability-dashboard.md)。
...
浏览器打开 `http://localhost:21680`。对外精简视图：`http://localhost:21680/?view=public`。

| 参数 | 默认 | 说明 |
|---|---|---|
| `--port` | 21680 | 也可用 `KARMACHAIN_DASHBOARD_PORT`。**刻意不在 protocol.json 里** —— 加进去会触发 stamp 全链重置，见 ADR-0010 |
| `--interval` | **5 秒** | 也可用 `KARMACHAIN_DASHBOARD_INTERVAL`。**硬上限 6 秒** —— 见下 |

> **间隔与发现时延的关系（改默认值前先读这一段）**
>
> 发现时延 ≈ **间隔 + 一轮探测的最坏耗时**，而"最坏耗时"取决于故障形态：
>
> | 故障形态 | 一轮探测耗时 | 默认 5 秒下的发现时延 |
> |---|---|---|
> | 容器被 kill（端口立即拒连） | 约 0.2 秒 | 约 **5.2 秒** |
> | 机器断电 / 静默丢包（等满探测超时） | 约 4 秒 | 约 **9.1 秒** |
>
> 10 秒预算（FR-018）下，默认 5 秒的最坏余量只剩约 **0.9 秒** —— 这是 2026-09-10
> 从 2 秒调到 5 秒的代价。**因此硬上限 6 秒不能再放宽**：7 秒就会在静默丢包时超出预算。
> 要更快发现就把间隔调小；超过上限时脚本**拒绝启动**而不是悄悄夹取（悄悄改成合法值
> 会让你以为自己设的间隔生效了，而实际发现时延与预期不同）。
| `--deployment` | 活动形态 | 与其余 `devnet-*` 同名同义 |

面板可从**任一台**承载节点的机器打开，刻意不设"监控机" —— 那台机器一挂，面板就与
"链停了"无法区分。宿主唯一前置依赖仍是 Docker（面板跑在既有 `karmachain/verify:local`
镜像里）。

### 10.2 三档健康度：先读这张表

这是面板最容易被误读的地方。容错上限由共识参数决定：查询门槛 α/k = 15/20 = **75%**，
等权验证者 n 个时可容忍 f ≤ ⌊n/4⌋。当前 n=5 → f=1。

| 参与共识 | 健康度 | 面板档位 | 实际状态 |
|---|---|---|---|
| 5/5 | **100%** | 正常 | 出块中，**还能再掉 1 个** |
| 4/5 | **80%** | **高危：零余量** | **仍在正常出块**，但余量为 0 |
| 3/5 | **60%** | **已停止**（显目报警） | 低于 75% 门槛，停止出块 |

**「低于 80% 就是链已停止」是错的判据。** 80% 恰是"最后一格仍在工作"；真正的停止线
在 75%，即掉到 3/5 = 60% 才停。

**百分比本身从来不是判据。** 同一判定式在 n=9 / f=2 时给出的落点是：8/9 = 89% 仍
`normal`、7/9 = 78% 才 `zero-margin`、6/9 = 67% 是 `stopped`。面板里没有任何写死的
75/80/60（由 `tests/unit/dashboard-no-hardcode.test.mjs` 守着）。

另有两个**非健康度**状态，它们是"此刻不该用健康度回答"的处境：

| 档位 | 什么时候 | 面板会说 |
|---|---|---|
| **启动中** | 尚未达到门槛，且缺口全由引导中的节点造成 | 还在等哪些边界，**不会**说"链已停止" |
| **失去观测能力** | 本机一个节点都探不到 | 先检查本机网络，**永不**说"链已停止" |
...
|---|---|
| **验证者级** | 还能掉几个验证者 |
| **边界级** | 还能整体失效几个**有效**故障边界 |

两者可以不同：若两个验证者挤到同一台机器，验证者级余量仍是 1，但边界级余量变成 0
（那台机器一挂就同时掉两个）。**只看前者会高估冗余。** 边界级余量按并查集之后的
*有效*边界算 —— 按声明的边界算会得到一个「在现实里为假的绿灯」（共享供电、共享交换机
会被掩盖）。

### 10.3.1 「恢复能力已丧失」是另一回事 —— 它和档位正交

面板除了三档健康度，还会在某个条件下多出一条提示：

> ◆ **恢复能力已丧失** —— 链仍按上面的档位出块，但此刻**任何 L1 验证者一旦重启
> 都无法重新加入**。

**它不是档位，也不改变档位。** 链在正常出块时档位照样是 `normal / 100%`，
两个余量一个都不变 —— 变的只是多了这条提示。两件事可以任意组合，
而**最要紧的那个组合是「正常出块 + 无法恢复」**：它看起来完全健康。

| 在服务 P 链的 Primary 数 | 提示 |
|---|---|
| 2 | 不出现 |
| **1** | **出现** —— 一个不够，见 [§9.5 的门槛表](#95-断电重启后的恢复) |
| 0 | 出现 |

**判据是"少于两个"，不是"一个都没有"。** 两个 Primary 各握 P 链一半权益，
而引导要求连上足够的权益 —— 少一个就到不了门槛。

#### 看到它该做什么

1. **先把两个 Primary 都启动** —— 只起一个不够
2. **在那之前不要重启任何验证者** —— 重启了就回不来，要等到两个 Primary 都在
3. 已经在跑的验证者**不受影响**，链照常出块；卡住的那个在两个 Primary 都回来后
   **约半分钟自行追上**，不需要再动它

顺序与 [§9.5「恢复顺序」](#95-断电重启后的恢复) 完全一致 ——
那一节是"事后查"的地方，这条提示是"当场看"的地方。两处的门槛数字与顺序
由 `tests/unit/recovery-docs-parity.test.mjs` 锁住，不会各说各话。

#### 它**不**意味着什么

**不意味着数据有风险，也不需要重置链。** 卡住的验证者只是没引导完，
它的数据卷完好；链本身连块都没少出。
（003 的 V-04 实证过更极端的情形：链**停摆**期间一笔 45 秒拿不到回执的交易，
在验证者恢复后被**原样打包**进区块 —— 未重发、未改 nonce。）

#### 观察者失明时它不说话

本机连不上任何节点（档位为"观察者失明"）时，这条提示**不出现** ——
面板此刻不知道那些 Primary 是死了还是只是看不见，两个方向的断言都可能错。
说"已丧失"会让人在网线松的时候不敢重启任何东西；说"正常"会在真的全停时给出假绿灯。
**所以它选择不说。**

### 10.4 十个场景的读法

完整的验证手册见
[specs/003-chain-health-dashboard/quickstart.md](../specs/003-chain-health-dashboard/quickstart.md)。
下面是各场景在面板上**应当**呈现的样子，以及最容易误读的地方。

| 场景 | 面板应当显示 | 别误读成 |
|---|---|---|
| A 正常态 | 100% 正常，余量 1/1，五台创世哈希一致 | — |
| B 停 1 个验证者 | ≤10 秒变 80%「高危：零余量」，文案写明**链仍在出块** | 「链已停止」 |
| C 停 2 个验证者（需两台机器） | ≤10 秒变 60%「已停止」，说明成因与"至少再恢复 N 个" | 「数据可能丢失」「需要重置」—— 都不对，这是**安全停摆** |
| D 单链路故障 | 该节点「**本机视角不可达**」，健康度**保持 100%**，无报警 | 「节点下线」。002 的 ubuntu-1 线缆丢包就是这一类 |
| E 观察者本机断网 | 「**失去观测能力**」+ 逐边界的路径佐证 | 「链已停止」 |
| F 两个 Primary 全停 | L1 健康度**仍 100%**，Primary 单独成组显示不可用 | 「7/7 节点不健康」。它们不参与 L1 出块 |
| G 空闲十分钟 | 全程 100% 正常，高度不动，**无报警** | 「链卡住了」—— 本链按需出块 |
| H 面板运行 30 分钟 | 高度**零增长** | 若涨了，说明自动路径在写链，那会毁掉 G 的判据 |
| I 精简视图 | 只有链 ID / RPC 端点 / 高度 / 健康度 | 它现在的用途是"可以截图给外部看"，**不是**"第三方自己来访问" |
| J 全网分批启动 | 全程「**启动中**」并列出未就绪的边界 | 「已停止」「须处置」—— 先起来的机器上无可处置之处 |

### 10.5 面板刻意**不**做的四件事

| 不做 | 为什么 |
|---|---|
| 不请求 `/ext/health` | 5 个 L1 验证者带 `partial-sync-primary-network=true`，综合健康位含 P 链可达性 —— 两个 Primary 一停就会把 5 个**工作正常**的验证者判成不健康（002 实测；2026-09-10 又原地复现：l1-1 返回 503 而它自己 `healthy/748`）。判据以"本节点能否参与 L1 出块"为准。详见 [§3.5](#35-重置与状态保留) 的观测盲区一节 |
| 不调用 docker | 核心判据不得依赖容器运行时。容器事实只是**可选增强**（读 `devnet-status` 写出的那份，带 120 秒 TTL），缺失时降级为纯网络判定并**显式告知** |
| 不对节点做任何操作 | 重启/停止/改配置一律不做 —— 那是 `scripts/devnet-node` 的职责 |
| 自动路径不写链 | 自动探活会持续产生区块，使"高度"不再反映真实业务活动 —— 而"按需出块、高度停滞不是活性信号"正是一条诊断依据。探活只能由人点按钮触发，且点击前会告知它会消耗开发账户余额 |

前三条由 `tests/unit/dashboard-boundaries.test.mjs` 静态守卫（每条都自带反向用例，
证明它确实会变红）；第四条由 `tests/e2e/dashboard-readonly.test.mjs` 守。

#### 代理容器的健康位不回答"链能不能用"

`docker ps` 里 `karmachain-rpc-*` 显示 `healthy`，**不代表链可用** ——
它只回答"这个代理容器本身有没有在正常工作"（进程活着、配置解析通过、端口在监听）。
全部验证者都不可服务时它**仍然是 healthy**，因为代理确实在正常工作。

这是刻意的（[ADR-0011](adr/0011-proxy-health-answers-only-for-itself.md)）：
它原先打的是 avalanchego 的**综合**健康位，而那个位含 P 链可达性 ——
两个 Primary 一停就 503，nginx 把 503 计为上游失败，一次探测毒遍五个上游，
探测间隔又短于惩罚期，于是**真实客户端流量一起吃 502 而链好着**（2026-09-10 实测）。

**"链能不能用"看面板的档位，或跑 `scripts/devnet-verify`。**

### 10.6 排障

| 症状 | 原因与处置 |
|---|---|
| `找不到运行中的 karmachain-rpc-<边界>` | 面板要接到节点所在的容器网络上 —— 先 `scripts/devnet-start` |
| `--interval Ns 超出上限 6s` | 这是刻意拒绝而非夹取：悄悄改成合法值会让你以为自己设的间隔生效了 |
| 页面显示「数据已陈旧 N 秒」 | 采集失败但服务还活着。看服务端 stderr 的 `dashboard: 采集失败` 行 |
