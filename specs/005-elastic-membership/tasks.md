...
- [X] T045 [US4] 实施选定方案
      —— **2026-09-26 活链完成**。P 链权益持有者 **2 个各 50% → 6 个各 16.66%**，
      掉任意一台机器剩 **83.34%**（门槛 80%）。四笔 `AddPermissionlessValidatorTx`
      各质押 100 万 AVAX、365 天：`JzTiPiY…`(l1-5) / `2RhaDtZ…`(l1-6) /
      `2V4ykJA…`(l1-7) / `28zyFg7…`(l1-8)。L1 侧全程未受影响（14/14、9/9、余量 2、
      创世哈希不变）。**004 的 V-08 记下的"两个 Primary 必须都在"这条依赖，
      至此第一次被真正解除。**
      **挑选改了**：四个全挑 Linux（ubuntu-3/4/5/6），而 T044 当初那组含两台 Windows。
      005 期间加进来的机器把「掉任意 1 台」与「重启后零人工」同时买到了 ——
      T044 判定为"当前硬件下无法同时达到"的那一条，前提已经变了。
      六台持有者机器逐台核过 docker 开机自启（ubuntu-1/2/3 是 snap 版、
      单元名 `snap.docker.dockerd.service`；ubuntu-4/5/6 是 apt 版），全部 enabled。
      **只写到"重启零人工"** —— "停电零人工"还要求 BIOS 的 Restore on AC Power Loss，
      六台都没核，另记待办。
      新工具 `tools/membership/add-primary-validator.mjs`（六条前置检查，
      第 ④ 条问活节点的 C/X 引导状态 —— 只有它能发现"文件改了而容器没重建"）。
      守卫 `primary-stake-distribution.test.mjs` 6 条，变红检查做过。
      **两条待办**：① 那个工具只能在装了 node_modules 的机器上跑，
      违背 README 的"宿主唯一前置依赖是 Docker"（用户在 ubuntu-3 上撞到）；
      ② 既有两个 Primary 的质押 **2027-09-08 到期**，比我们这四笔的 2027-09-26
      还早 18 天 —— 那天一到持有者掉回 4 个各 25%，本条买到的性质失效。详见 V-78。
- [X] T046 [US4] 改 `tools/dashboard/snapshot.mjs`：004 那个写死的门槛常量改为
      **按实际权益分布判定**，保留原注释意图（数字来源是权益门槛而非"总数"）（FR-023）
- [X] T047 [US4] **quickstart 场景 P（需五台）**：掉任意 1 个 P 链权益持有者时，
      一个被重启的 L1 验证者**仍能**完成引导并重新加入（**≤120 秒**，V-16 / SC-013）
      —— **2026-09-26 活链完成**。停掉持有者 `l1-8`（ubuntu-6），P 链在线权益
      降到 **83.33%**（≥80%）；随即重启 `l1-1`（win-1），**39.2 秒**完成引导并
      `participatesInConsensus = true`；该状态下发交易仍 `status=success`
      （区块 2511，4.1 秒确认）。逐秒：`05:26:06.686` 发令 → `05:26:14.578` 容器起
      → `05:26:15.941` 开始引导 → **`05:26:45.926` 引导完成**。
      **靶子选得对才算数**：`l1-1` 带 `partial-sync-primary-network`、**不是**
      P 链验证者，所以它的引导**恰恰依赖**"能连上 ≥80% 的 P 链权益" ——
      那正是 004 的 V-08 记下的依赖。对照：改造前 2 个持有者各 50%，
      掉 1 个剩 50% ⇒ 引导**失败**；今天同一个动作 39.2 秒完成。
      **同一个动作，改造前做不成，现在做成了。**
      过程中面板只报一条 `node-infra | l1-8 整域缺席`，分类与处置都对，
      没有把"正在引导的节点"误报成故障。恢复后核过：L1 余量回到 2、零异常，
      P 链在线权益 100%。
...
- [X] T056 [US6] **quickstart 场景 Q**：在一台新机器上只起代理 + 面板，
      确认面板可用、经它的入口能确认交易，且 `git status` **干净**（SC-014）

> **2026-09-23 现场完成**（新机器 192.168.1.41，**不在部署描述里**）。
>
> | 判据 | 结果 |
> |---|---|
> | `git status` 干净 | **空** —— 克隆之后、建镜像之后、起容器之后各查一次，三次都空 |
> | 经它的入口能确认交易 | 从 win-1 指向 `192.168.1.41:8545` 发一笔真交易 → 块 2232，551ms，零 5xx |
> | 高度与全网一致 | 2231 = 2231 |
> | 面板可用 | `tier normal`、`8/8`、余量 2、`observer.blind=false` 且**可达 10/10** |
>
> **F-8 的两条理由在一台完全不在声明里的机器上成立**：它的 IP 不在任何节点的
> `http-allowed-hosts` 里（那份清单只有 `127.0.0.1` 与 `localhost`），而节点照样应答 ——
> 代理把 Host 改写成 `localhost`，面板直连时用 IP 字面量而 avalanchego 对它无条件放行（V-19）。
>
> `containerFacts.available=false` 是**正确降级**：那台机器上本来就没有节点容器。
>
> **这一条真正的收获是它暴露的三个文档缺口**（见 research V-72），三个都已修。
...
- [X] T061 **quickstart 场景 R（SC-016）**：找一名**未参与本期**的人，
      只给他文档，让他把一台机器加成验证者。**卡住就改文档，不改判据**
      —— 2026-09-25 跑了**两轮**真人试验。第二轮那位把 `192.168.1.42` 加成了
      第 9 个验证者：链上 8→9、三侧一致、14/14 全绿（`9/9 validators online,
      tolerance 2, full margin`）、创世哈希不变、高度 2232→2247、
      既有节点容器 `Created`/`StartedAt` 逐字符未变。
      两轮共暴露 **7 条**（5 条文档 / **2 条产品**），全部已修并各带守卫。
      **这件事做完了，所以打勾；而 SC-016 那条判据没过** ——
      它要的是"不需要问人"，他问了两次、还撞上一个会让人无限循环的工具缺陷。
      SC-016 因此由 ❌ 转 **⚠**（不是 ✅），要转 ✅ 需第三个人从零跑通、全程不问。
      详见研究 V-74 / V-75。

> **2026-09-23：把"卡住就改文档"那一半先做了，判据那一半仍未做。**
>
> SC-016 要的是"**未参与本期**的人、不问人、不读源码"。写这套东西的人与
> 全程参与的人都不具备那个独立性 —— 让他们扮演新人只会得到一个假绿灯，
> 而那正是这一条要防的。**所以这一条仍未打勾。**
>
> 但补救条款（"卡住就改文档"）不必等人：把文档那条路与**真实走过两遍**的路
> （加第七台 ubuntu-5 / l1-7、加第八台 ubuntu-6 / l1-8）逐步对照，找出了四个缺口，
> 都已补进 `docs/devnet.md` §11.2（详见 research V-73）：
>
> | 缺口 | 后果 |
> |---|---|
> | 没写目标机器的前置依赖，也没警告 **snap 版 docker** | bind mount 静默失效，报错指向 avalanchego（实际耗了好几轮） |
> | 改声明写的是三处，**实际是五处** | 漏 `validators.count` 与 `local` 形态 → 校验器报错（好在它说得很清楚） |
> | **完全没有"同步到其余机器"这一步** | 漏了**不会报错**，而新节点永远进不了负载池 —— 最危险的一个 |
> | 没提 P 链高度滞后与 `--nudge` | 第三步必然卡一次 |
>
> ### 真要跑这一条时怎么跑
>
> **给他什么**：仓库地址、一台干净的机器、`docs/devnet.md`。**不给**：任何口头补充。
>
> **不给什么**：不要在他卡住时解释。他问，就记下来"他问了什么"——
> 那一条就是文档的缺口，**答完再记也不迟**。判据是"不需要问人"，
> 而一次被回答过的提问，已经证明了文档不足。
>
> **记什么**（这四项决定这一条算不算过）：
> 1. 他在**哪一步**停下来、停了多久
> 2. 他**读了源码没有** —— 读了就是文档没覆盖到那件事
> 3. 他**问了什么** —— 每一问对应一处要补的文档
> 4. 最终有没有**独立**完成（链上成员 +1、两侧一致、既有节点容器未重启）
>
> **卡住就改文档，不改判据。** 改完之后这一条要**换一个人重来** ——
> 同一个人第二次跑已经不是"未参与"了。
>
> ### 2026-09-25：第一轮真跑，第一条发现在**第 0 分钟**
>
> 一名未参与本期的人接手，目标机器 `192.168.1.42`（第 9 台）。
> 给的是仓库 + 机器 + `docs/devnet.md`，按上面的规矩不作任何口头补充。
>
> **他还没开始做就卡住了**，而卡住的原因不是某个步骤写错：
> **`docs/devnet.md` 有 1697 行，是一本"这张网络的全部知识"，不是一张"照着做"的单子。**
> 他要的是"从头到尾怎么把一台新机器加成第 9 个验证者"，
> 而那件事的步骤分散在 §11.2（主线）、§9.2（前置）、§5.4（退出码与坑）、§11.1（f 不变）里，
> 还要自己把 `<边界名>`、`<节点序号>`、两个端口这些占位符换成具体值。
>
> **这一条与前面四个缺口不同类：前四个是"少写了什么"，这一条是"形态不对"。**
> 而形态问题**只有真人能发现** —— 我和任何参与过的人读 §11.2 都觉得它完整，
> 因为我们**已经知道**该去哪几节取剩下的部分。这正是 SC-016 要一个真人的理由，
> 它在这一轮的第 0 分钟就兑现了。
>
> **补救**：新增 `docs/add-validator.md` —— 线性单子，⓪…⑩ 十一步，每步一个判据，
> 不解释为什么（为什么仍在 §11.2，单子引它、不复制它）。三处加了指路：
> README、§11 抬头、§11.2 抬头。顺带修掉 §11.2 里一处被 heredoc 吃掉的反斜杠
> （`scriptsdevnet-member.ps1`）。单子里的序号与端口**让读者当场从 `deployment.json` 算**，
> 不写死 —— 写死的话加完第 9 个它立刻过期，那正是本仓库一直在防的漂移。
>
> 单子比 §11.2 多出来的两处（都是走过之后才知道要有的）：
> - **⓪ 抓基线**：既有容器的 `Created`/`StartedAt`、创世哈希、当前高度。
>   ⑩ 的"既有节点未重启"要靠它证明，而**事后补不回来**。
> - **⑩ 四条验收**并列成一段，而不是散在正文里。
>
> **这一轮的状态：判据改了对象，计时重置。** 现在要验的是
> "照 `docs/add-validator.md` 能否独立走完"。按上面那条规矩，
> **严格说这一轮已经不干净了** —— 他读过旧文档、我改了文档。
> 所以这一条**仍不打勾**：要么这一轮走完当作"文档改后的一次自测"，
> 要么换第二个人跑新单子才算数。
