...
**仍未测**：整条流程的实际耗时，以及 `index-enabled=false` 在节点开始完整同步
之后是否仍然成立（那个 flag 当初是为了配合播种进卷的数据库设置，见
`render-node-flags.mjs` 的注释；改成完整同步是否触碰它那条 FATAL 未知）。
...
| 磁盘 / 带宽 | 每台多一个完整节点 | **≈ 0**（C/X 链为空） |
| 启动时间 | 不变 | **≈ 不变**（引导工作 4.7 秒，其余是心跳节拍） |
...
- **V-13** ⚠ **部分实测（2026-09-16，见 R-07a②）**：基线拿到了 ——
  l1-1 带 `partial-sync` 时数据目录 **4.69 MB**，而 **C 链与 X 链高度都是 0**。
  去掉 flag 要多同步的是两条零区块的链，**增量预期接近于零**。
  **仍缺对照实测**：去掉 flag 后同一节点的实际占用与稳定带宽。
  该测量要改配置并重建容器，按本阶段开头那条警告须先问用户。
  ⚠ 结论不可外推到主网（那里是数百 GB）。
- **V-14** ⚠ **部分实测（同上）**：`partial-sync` 基线为**初始化到引导完成 30.08 秒**，
  其中真正的引导工作 **4.7 秒**，其余 25 秒是 `healthCheckFreq: 30s` 的心跳节拍。
  P 链高度 5、缺失区块 1 个、执行 0 个（134µs）。**仍缺去掉 flag 后的同一计时。**
- **V-15** ⚠ **流程已查清、耗时未测（见 R-07a③④）**：
...
- **V-34** ✅ **ACP-77 第一步已在真链上执行**（2026-09-15）。
