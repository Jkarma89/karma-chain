...
### V-76 一个落后的节点会让 `devnet-verify` 报成"链坏了"（2026-09-25）

T061 收尾之后、把面板起回来时撞到的。**与成员变更无关，但同属"判据说的是一回事、
它实际测的是另一回事"那一族**，所以记在这里。

#### 现场

`l1-3` 有过一次 P2P 退化（只连上 4 个对端、投不了票），重启后恢复。随后跑收口检查：

```
[OK]   node              11/11 nodes serving
[OK]   validator         9/9 L1 validators bootstrapped, peers>=4 each (observed 9/10)
[OK]   fault-tolerance   9/9 validators online, tolerance 2, full margin
[FAIL] transfer          [category: rpc] Timed out while waiting for transaction … to be confirmed
[SKIP] receipt           no transfer to inspect
[FAIL] block-production  [category: rpc] Timed out …
[FAIL] contract          [category: rpc] Timed out …
KarmaChain is NOT READY   (14 checks, 3 failed)
```

读起来是"链收不了交易"。**而那三笔交易全都进链了** —— 其余八个节点的高度
从 2249 涨到 2252，**正好三块，就是它发的那三笔**。

真实故障是：`l1-1` 卡在 2249 不动（`catching-up`，45 秒零进展），
而 `devnet-verify` 经本机 RPC 代理读，代理把读请求发给了它。
**交易写成功了，回执读不到。**
...
- 它说 `category: rpc` / 交易超时 → 把人引向"RPC 坏了 / 链停了"（**已修**：现在归 `transaction`，并在端点落后时改判 `node` 且指名道姓）
- 真实的事实是 `l1-1 落后 3 块` → 处置是修那一个节点（§5.3），不是查 RPC

同一时刻面板给的是 `sync-lag | l1-1：落后 3 块，5s 采样窗口内无进展`，
**分类准确、处置指向正确**。两个工具看同一件事，一个说对了一个说错了。

这与 V-74 的第 5 条是同一族的反面：那次 `devnet-verify` 对
"声明 9、链上 8" **判绿**（该红不红），这次对"一个节点落后"**判成链故障**
（该红，但红错了地方）。**共同的根因：它经代理读，而代理会指向任意一个上游。**
ADR-0011 已经为代理的健康位定过"只为自己作答"，但**读链**这条路仍然继承了
"随便哪个上游"的语义。

#### 现在做了什么、没做什么

**做了**：`docs/devnet.md` §6 加一行 —— 三条交易类检查一起红、而
`node`/`validator`/`fault-tolerance` 全绿时，先比各节点高度，
只有一个落后就说明交易已经进链，去修那个节点。

**没做**：没有改 `devnet-verify`。可选的改法是让它在交易超时时**顺手比一次各节点高度**，
发现"只有一个落后"就把类别从 `rpc` 改判为 `node` 并指名道姓。
那是产品决定，未动代码。

#### 当天补：两处一起改了（都动了代码）

**① 该红不红**（V-74 第 5 条）。`tools/verify/checks/basic.mjs` 的 `splitNonMembers`
再分一次桶：`registering`（还在跑、却不是链上成员 ⇒ **活儿没干完**）与
`stale`（进程已停 ⇒ 已被移除，只剩声明要清理）。前者由 `validator` 项**判 FAIL**，
后者维持 OK 并照旧说出来。

判据用「**那台机器还活着吗**」，而不是再去读一次合约 —— 退出的规程规定
"先从集合移除 → 再停进程"，所以被移除的那些进程是停的；正在注册的那些必须在跑。
**它会误判的那一种情形（紧急摘除之后机器又被开机）写在代码注释里，没有藏起来。**

**② 红错地方**（本条）。`categorizeError` 把「等交易确认超时」**提前**判为
`transaction`（原先被通用的 `timed out` 规则吞进 `rpc`）；随后 `verify-network.mjs`
再问一句**"写进去了没有"**：比对我们读的那个端点的高度与各节点自报的最高高度，
端点落后就改判 `node` 并附一句指名道姓的诊断 ——
「端点落后全网 N 块，交易多半已经进链，读不到的是回执，落后的节点：`l1-1@2249`」。
诊断拿不到数就**不作答**（返回 null）：猜出来的诊断比没有诊断更坏，
它会把人引去修一个没坏的东西。

守卫 `tests/unit/verify-membership-and-lag.test.mjs`（12 条），
既有 `verify-non-members.test.mjs` 加一条「判红分支必须存在」。
**三条变红检查都做了**：去掉 `alive` 判据 → 判红那条红；删掉提前判定 → 类别那条红；
把判红分支改成 `if (false)` → 新加那条红。还原后 1593 全绿，活链复跑 14/14。

改这两处时**既有守卫抓到我三次**，都抓对了：`no-hardcode` 抓到我在测试里写了真实
RPC 端口；`verify-non-members` 的两条抓到我改了调用签名与文案。
其中那条按 `nonMemberNote(nonMembers)` 这串字面量计数的断言，
**把判据钉在了一个参数名上** —— 那不是它要守的东西，已改成数调用本身。

#### 我自己踩的那一脚，一并记下

我中途连读三次高度都得到 2249，于是说了一句"链可能不出块了"。
那三次读**全落在同一个落后节点上**。§5.3 ③ 早就写着
「本机的 RPC 代理可能正指向这个落后的节点 —— 那会让**所有**读数变陈旧」，
我照样踩了进去。

**教训不是"要小心"**，而是：**判断"链是否在出块"时，单一入口的连续读数不构成证据** ——
连读三次只是把同一个谎言听了三遍。要么直连多个节点比高度，要么看面板
（它本来就是直连各节点的）。这一条与 004 的"恒定非空的告警等于没有告警"同源：
**一个不会变的读数，不因为你多读几次就变得可信。**

---

### V-77 第一次整轮全绿，以及它暴露的"没跑过 ≠ 跑过并通过"（2026-09-26）

本期第一次出现**从头到尾零失败**的完整 e2e：**49 条全通过，82 分钟**。
在此之前四轮分别是 10 / 2 / 1 / 2 个失败，上一轮（2026-09-25）是 4 个。

两个 30 分钟观测窗口都**真的跑满了**：场景 F（边界 win-1 整体失效）30 笔全确认、
高度 2417→2447；SC-003（单验证者离线）30 笔全确认、0 次失败、`l1-1` 全程离线、
高度 2454→2484（正好 30 块，一笔一块）。

#### 但真正值钱的是上一轮那 4 个失败里的第一个

`single-validator-window.test.mjs` 报 `ReferenceError: pickLocalVictims is not defined`。
`pickLocalVictims` / `localVictimSkip` 在一次改动里从导入列表掉了出去，
而它们在模块**顶层**被调用 —— 整个文件在 import 阶段就炸了，**一条断言都没跑**。

要害不在这个笔误。**在 e2e 的汇总里它只是 `# fail 1`，
和"跑了一条、红了一条"长得一模一样。** 而这一份跑的正是 SC-003 的正式验收。
它悄悄缺席了整整一轮，而总数看不出来。

**没跑过的测试与跑过并通过的测试，在总数上是分不出来的。**
这与本期反复出现的那条同源（"判据说的是一回事，它实际测的是另一回事"），
但更隐蔽一层：那些是**测错了东西**，这个是**根本没测**。

守卫 `tests/unit/e2e-imports-resolve.test.mjs`：查"导入了库里却没有"（加载时
SyntaxError）与"用到了却没导入"（加载时 ReferenceError）两个方向。
只认调用点并排除本文件自己声明的同名符号 —— 按裸标识符查会被字符串与局部变量误伤，
而**一条会误报的守卫很快会被人关掉**。放在单元测试里几毫秒就能答，
而靠跑一轮 e2e 来发现"某个文件根本没加载"要一个多小时。

这条守卫自己先红了一次，红得对：第一版的导入块正则 `[\s\S]*?` 虽然是懒的，
却会从文件里**第一个** `import {` 一路吞到 devnet 那一行的 `}`。
更值得记的是：当时"库里解出 ≥20 个导出""≥10 个文件在导入"这两条前提断言
**照样通过** —— 解出来的"名字"里带着换行和 `from 'node:test';`，而数量都够。
**数量对不代表解析对。** 已补一条断言：解出来的必须长得像标识符。

#### 起跑前的前提是被检查过的，不是碰巧

第一次尝试重跑时 `l1-2` 卡在落后 90 块、采样窗口内零进展（peers 9 个、
容器健康检查仍是绿的）。**没有开跑** —— 带着一个未追平的验证者起跑，
`requireFullMargin` 会让 12 个破坏性套件整批判红，那是假失败，会把真结果淹掉。
重启 `l1-2` 后追平到 2357、余量回到 2、零异常，才起跑。

**这是当天第二次"重启/高负载之后节点卡住而非追赶"**（前一次是 `l1-1`，见 V-76）。
两次形态一致：对端数足够、容器健康检查绿、高度长时间不动、重启后数秒追平。
两个数据点还不足以定性，**记在这里等第三次** —— 若再现，它就不是偶发。

#### 跳过的 6 个仍然是跳过

4 个是跨机形态的必然跳过（docker 只能操作本机容器，而 T-5 保证每边界至多 1 个验证者，
没有单台机器能同时停掉 2 个），2 个要显式开关。
`run-tests` 把它们单独列出来而不是混进通过里 —— **"跳过"不是"通过"**，
这一点在汇总里必须看得见。

---

### V-78 F-7 落地：P 链权益从 2 个各 50% 变成 6 个各 16.66%（2026-09-26）

004 的 V-08 记下的那条依赖 —— **"两个 Primary 必须都在，掉一个引导就失败"** ——
今天第一次被真正解除。

| | 之前 | 之后 |
|---|---|---|
| P 链权益持有者 | 2 个，各 50% | **6 个，各 16.66%** |
| 掉任意**一台机器** | 剩 50%（门槛 80%）✗ | 剩 **83.34%** ✓ |
| 持有者所在机器 | ubuntu-1、ubuntu-2 | 六台 Linux，**每台恰好一个** |

四笔 `AddPermissionlessValidatorTx`，各质押 100 万 AVAX、365 天：
`JzTiPiY…`（l1-5）/ `2RhaDtZ…`（l1-6）/ `2V4ykJA…`（l1-7）/ `28zyFg7…`（l1-8）。
L1 侧全程不受影响：14/14、9/9 验证者、余量 2、创世哈希不变。

#### 挑选改了：四个全部挑 Linux，而 T044 当初挑的含两台 Windows

T044（2026-09-16）写着「**当前硬件下"掉任意 1 台"与"停电后零人工"无法同时达到**，
要同时达到需要第 5 台会自动恢复的机器」。那是当时只有 5 台机器的结论 ——
四个兼任名额里必须含 `l1-1`(win-1) 与 `l1-2`(win-2)，而 Windows 的容器运行时
要用户登录才启动（ADR-0006）。

**005 期间加进来的那几台把这个约束解掉了**：现在有 7 台 Linux，其中 5 台不带 Primary。
挑 `l1-5`/`l1-6`/`l1-7`/`l1-8`（ubuntu-3/4/5/6），六个持有者全落在 Linux 上。
同样的代价、同样的数量，多买到一条性质。

**刻意不选 `l1-9`(ubuntu-7)**：那台前一天才加入，没核过 systemd 状态。
**四个名额用已核过的，不用没核过的。**

#### 那条性质只写到"重启零人工"，没写"停电零人工"

逐台核过六台：ubuntu-1/2/3 是 **snap 版 docker**（单元 `snap.docker.dockerd.service`），
ubuntu-4/5/6 是 apt 版（`docker.service` + `docker.socket`），**六台全部 enabled**；
节点容器全部 `restart=unless-stopped`。所以**重启之后零人工**成立。

**"停电之后零人工"不写** —— 它还要求断电恢复后机器自己开机
（BIOS 的 Restore on AC Power Loss），那一条六台都没核。
这不是 F-7 引入的新风险：今天的两个 Primary 就在 ubuntu-1/2 上，本来就依赖它。
另记待办。

#### 判据第一版是错的，而它错的方式很典型

第一版让人跑 `systemctl is-enabled docker`。在 snap 版机器上它答
`Failed to get unit file state for docker.service: No such file or directory` ——
读起来像"没有开机自启"，**而实际是开的，只是单元叫 `snap.docker.dockerd.service`**。

**判据把"查不到"当成"否"，就会得出一个在现实里为假的结论。**
与本期那条"无法测量 ≠ 测量结果为否"同源。改成按单元文件列表判，不假设单元叫什么。

#### 工具：`tools/membership/add-primary-validator.mjs`

六条前置检查，任一条不过退出 30（一步都不动链）。其中第 ④ 条是重点：

> ①②③ 查的都是**仓库里的文件**，而"文件改了、容器没重建"这种情况三条全过。
> 只有问那个节点自己「你在全量同步主网络吗」（`info.isBootstrapped` 的 C/X）才能发现。

本轮拿到了干净的 A/B：改过的 `l1-5` 报 `C=true X=true`，未改的 `l1-6` 报 `false`。
**这是行为层面的证据，比 grep 配置文件强** —— 后者看不出进程有没有重读配置。

BLS 的 `publicKey` 与 `proofOfPossession` **向活节点索取**（`info.getNodeID` 的 `nodePOP`），
不从声明里读、更不用 `signer.key` 自己算：`l1-5` 是创世验证者、声明里根本没有
`identity` 块；而向节点本人要，顺带验证了"要质押的身份"与"正在跑的那个进程"是同一个。
与声明里已有的三个交叉核对过，一致。

影响预测按**机器**算而不按持有者算（守卫 `primary-stake-distribution.test.mjs`
里有那个 75% vs 87.5% 的对照夹具）—— 按持有者算会给出在现实里为假的绿灯，
那正是 T044 否掉"全部 6 个都兼任"的理由。

#### 一颗定时炸弹，绑定日期不是我们这四笔

我们这四笔 **2027-09-26** 到期。而查 P 链时发现：
**既有两个 Primary 的质押 2027-09-08 到期。**

那天一到，持有者从 6 掉回 4、每个 25%，掉一台剩 75% < 80% ——
**F-7 买到的性质会在 2027-09-08 失效，比我们这四笔还早 18 天。**
链上不会提醒，面板也不会（它看的是当前分布，不是到期日）。

取 365 天（上限）而不是更短，正是为了不让这个日期来得更早：
`minStakeDuration` 只有 24 小时，取短的话**明天**就会悄悄退回去。

#### 两处我自己的错

① 工具第一版按 `validatorIndex` 去 `validators.nodes[]` 里找声明项，而
`deriveTopology` 合并之后节点对象上**没有**那个字段 —— `find` 返回 undefined，
报错报在 `identityOf` 里，指向身份库，**而真正错的是我这里的匹配键**。

② 交付的工具**只能在装了 `node_modules` 的机器上跑**。README 承诺
"宿主唯一前置依赖是 Docker"，`devnet-member.sh` 正是为此把工具跑在容器里。
用户在 ubuntu-3 上跑它、撞到 `Cannot find package 'ajv'` —— 撞的就是这个缺口。
补法两条（给 `devnet-member` 加第四个子命令 / 新开一对 `devnet-primary.{sh,ps1}`），
倾向后者，不动既有契约。**未做，记为待办。**

#### 还没做的那一半

`FR-022` 的判据是"**实施后**掉任意 1 个持有者仍能加入"。权益分布现在支持它了，
**但那是算术，不是实测** —— 真正的实测是 T047（停一台持有者的机器，
再重启一个 L1 验证者，看它能否在 ≤120 秒内引导并重新加入，SC-013 / V-16）。
所以 FR-022 由 ❌ 转 **⚠** 而不是 ✅，SC-013 维持 ⚠。
**算术成立不是实测成立**，这是本期反复出现的那条分界。

> **当天晚些时候（T047）把这一半补上了，两条都已转 ✅ —— 见 V-79。**

---

### V-79 T047：把"掉一个持有者仍能加入"从算术变成实测（2026-09-26）

V-78 结尾那句"算术成立不是实测成立"当天就兑现了。

#### 做法与逐秒的账

停掉权益持有者 `l1-8`（ubuntu-6）→ P 链在线权益 **83.33%**（6 个持有者里少 1 个）。
随即重启 `l1-1`（win-1）并计时：

| 时刻 | 事件 |
|---|---|
| `05:26:06.686` | 发出重启命令（t0） |
| `05:26:14.578` | 容器启动 |
| `05:26:15.941` | P 链与 L1 链**开始**引导 |
| **`05:26:45.926`** | **引导完成**（readiness 与 health 的 `bootstrapped` 同时转通过） |

**39.2 秒**，判据 ≤120 秒。随后 `participatesInConsensus = true`，
该状态下发一笔交易仍 `status=success`（区块 2511，4.1 秒确认）。

#### 靶子选得对，这条实测才有意义

`l1-1` 带 `partial-sync-primary-network`、**不是** P 链验证者 ——
它的引导**恰恰依赖**"能连上 ≥80% 的 P 链权益"，那正是 004 的 V-08 记下的依赖。
若拿一个兼任了 P 链验证者的节点（`l1-5`…`l1-8`）当靶子，测到的就不是同一件事。

| | 改造前（V-08 实测） | 今天（T047） |
|---|---|---|
| 持有者 | 2 个各 50% | 6 个各 16.66% |
| 掉 1 个后在线权益 | 50% | **83.33%** |
| 此时重启一个验证者 | **引导失败** | **39.2 秒完成** |

**同一个动作，改造前做不成，现在做成了。** 这是 F-7 的全部价值，第一次以实测形式出现。

#### 面板在这一轮里表现正确

全程只报一条 `node-infra | l1-8：边界 ubuntu-6 的全部节点都不应答 —— 整域缺席，去看那台机器`。
分类与处置都对，**没有把"正在引导的 l1-1"误报成故障** —— 那正是 003 的
`participatesInConsensus` 与"catching-up 不是故障"那两条在起作用。
恢复 `l1-8` 之后连读三次：余量回到 2、零异常、P 链在线权益 100%。

#### 一处我自己的操作失误

计时脚本把重启和轮询写在一起，输出中途断了，**没拿到精确秒数**。
没有估、也没有用"看起来很快"糊过去 —— 改从容器日志里取
`check started passing {"name":"readiness","name":"bootstrapped"}` 的时间戳，
那是节点自己写下的、不受我脚本影响的事实。上面那张逐秒表就是这么来的。

**这件事本身是个小教训**：计时这种一次性的观测，把"发令"和"读数"耦合在一个脚本里，
脚本一出问题**观测就没了，而动作已经发生**（链上状态已经变了，不能重来一次拿数据）。
可重放的事实来源（日志、链上记录）比一次性的脚本输出可靠。

---

### V-80 SC-009 活链验证：拦是拦住了，但拦的方式是个缺陷（2026-09-26）

#### 做法：造一个真的违规，先自证，再跑真实入口

在运行中的 9 成员链上，把 `l1-8`、`l1-9` 并进 `ubuntu-3` 那个边界（连同原有的 `l1-5`
共 3 个，而 ⌊9/4⌋ = 2）。**先自证构造真的违规**再往下走 ——
否则后面的断言测的是别的东西（`membership-preflight` 的注释里早写过这一条）。

结果：加入**被拦下**，理由是

> `[T-5] 形态 "lan"：故障边界 'ubuntu-3' 含 3 个 L1 验证者，上限为 2（9 个等权验证者，
> 查询门槛 75% → 最多容忍 ⌊9/4⌋ = 2 个离线）。把 l1-9 移到另一个边界，或增加边界数量。`

**这段文案是好的**：点名了边界、给了上限的推导、说了该怎么改。
链一点没动：L1 成员仍 9、容器 `StartedAt` 逐字符未变、还原后 `render:check` 10/10 干净。

#### 缺陷：好文案用错了形式

它是以**未捕获异常 + 堆栈 + 退出码 1** 报出来的：
...
① **退出码 1。** 契约里 **30 = 前置检查未通过、一步都没动链**，而这正是那种情形。
   靠退出码分流的调用方会把它与未知故障混为一谈。
② **堆栈把人引向 `load.mjs:412`**，而真正要改的是 `blockchain/deployment.json`。

**这是本期已经记过一次的形状。** T031 的注入 ① 发现"入口不可达时抛原始堆栈、
退出码 1"并修掉了 —— 当时修的是**那一处**，不是这一族。所以这次修在共用的一层：
新增 `tools/membership/load-or-exit.mjs`，四个成员工具都走它。
实测复验：同样的违规现在退出码 **30**、无堆栈、原文案一字不改地保留。

守卫 `membership-load-or-exit.test.mjs`（6 条）。其中第 ⑥ 条测的不是这一层本身，
而是**四个工具真的都走了它**（扫源码，禁止再直接调 `loadProtocol()`）——
否则修好的是一个没人用的函数。变红检查：去掉 try/catch → 前四条立刻红。

#### 顺带查实的两件事

**一、真实入口走不到 `add-validator` 自己那条 T-5 检查。** `loadProtocol()` 里有同一条
约束且更早、报得更具体，所以 `add-validator.mjs:263` 那一支在真实路径上永远到不了。
**没有删它**（`preflight()` 是导出的，可以被传入手搓的 config，单元测试就那么用），
但在它旁边写明了：**这不是 T-5 的执行点，执行点在 `tools/protocol/load.mjs`**。
一个让人以为"这里在把关"的分支，比没有它更容易误导。

**二、入口脚本在工具跑起来之前就要求聚合器。** 第一次跑 `scripts/devnet-member.sh add`
得到的是退出码 10「找不到运行中的 karmachain-aggregator」——
**一个必然被拒的加入，仍然要求先起一个按需容器**。T-5 违规写在声明里，
不需要任何聚合器就能知道。这不是错误（依赖检查本来就在前面），但它让
"拒绝"这件事的成本高于必要。记下来，未改。

#### SC-008 为什么不能照做

同一天顺手查清了 SC-008（会让链跌破门槛的退出被拦下）在当前拓扑上**造不出来**。
拦截条件是 `withinBefore && !withinAfter`，而退一个成员只有在 `f` 真的下降
（跨过 4 的倍数）时才可能跌破。用真实的 `removalImpact()` 穷举：

| n | 7 | **8** | 9 | 10 | 11 | **12** |
|---|---|---|---|---|---|---|
| 能否造出拦截 | 否 | **能** | **否** | 否 | 否 | **能** |

链现在是 n=9 —— **不是"没去做"，是数学上不存在那个条件**。
要做得先退到 8（一次真实退出）、造 2 个离线（零余量窗口）、被拒之后再加回来
（又一轮质押与注册）。代价与收益不成比例，故维持 ⚠ 并把理由写进 dod，
而不是含糊地记一句"活链未造"。

**"做不了"与"没去做"是两件事，档位一样而信息量差很多。**
