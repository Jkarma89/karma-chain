...
- **V-08（两个 Primary 的真实作用，2026-09-10 实测）** ⚠️ **发现一个恢复能力上的单点**：
  ...
  **恢复这一半也测了：两个 Primary 齐了，卡住的验证者会自愈，无需人工干预。**
  primary-2 起来后 **约 36 秒**内（采样间隔 4 秒，故落在 32–36 秒之间）l1-1 依次完成：
  P 链引导 `false → true` → L1 那条链**在该节点上被创建**并引导完成 → 高度对齐 835 →
  面板 `zero-margin/80%` 回到 `normal/100%`。五个视角随后一致给出
  `normal / 100% / 余量 1 与 1 / 参与 5 / 异常 0 条`，经代理入口的交易确认于**区块 836、4111 ms**。

  顺带闭合了另一条：**代理那个 502 缺陷（dod.md 第六节）随 Primary 恢复自行解开** ——
  综合健康位转回健康，nginx 的惩罚期不再被 healthcheck 续上。也就是说那个缺陷的
  触发条件确实是"两个 Primary 全停"，不是别的什么。

  **完整的恢复顺序（可直接抄进运维文档）**：

  1. 先把**两个** Primary 都拉起来（少一个都不行，见上表）
  2. 再拉需要重新加入的验证者；已经在跑的验证者不用动
  3. 等约半分钟，面板会自己回到 `normal`；**不需要点任何按钮，也不需要重启面板**

  反过来做（先拉验证者）会白等 —— 2026-09-10 这一轮就是先拉了一个 Primary，
  等了几分钟毫无进展，直到第二个 Primary 起来才动。
