...
| 八 | 测试和验证优先 | ✅（**经 analyze 修补后**） | 每条 FR 都有对应判据；**每条新守卫都要做变红检查**（FR-027），结果记进 dod。契约 §8 列了 8 行变红核对表。**这一行原先是一句没核过的断言** —— `/speckit-analyze` 的机械覆盖扫描查出 **FR-019**（观察者失明时不作断言）与 **FR-030**（零新增依赖）**两条一个判据都没有**，前者只有实现与渲染、没有任何行为断言。已补 T046 / T047 与 T044 的依赖 diff；补完这句话才成立 |
...
| 十二 | AI 辅助开发治理原则 | ✅ | 全程 Spec Kit；research 逐条记决策与被否方案。两处**规格措辞问题**（原 FR-002 后半句、FR-017）**提出交人拍板而非默默改小 FR**，2026-09-11 已拍板并落地 —— 拍板过程又牵出**四处连带矛盾**（原 FR-008 / SC-007 / US1 场景 4 / 一条边界情形），一并改正。**这恰好证明了"提出来"的价值**：若按自己的理解实现，那四处矛盾会一直留在验收标准里 |
| 十三 | 依赖和技术选型原则 | ✅ | **零新增依赖**。R-01 刻意选了不需要 busybox `wget --post-data` 的方案 |
| 十四 | 文档与架构决策必须可追踪 | ✅ **已补**（2026-09-11） | [ADR-0011](../../docs/adr/0011-proxy-health-answers-only-for-itself.md)：完整成因链（七环，每环有日志证据）、四条为何不、三个参数一律不动的理由，以及 `zone` 那条观察。已进 `docs/adr/README.md` 索引 |
...
- 契约 §2 的 **C-4** 与 **V-02** 专门管这件事。判据已随 R-03 拍板定下：
  验的是**"nginx 自己死了或配置坏了会红"**（杀进程 / 改坏配置两条）
- 同时要有**反向确认** **C-5 / V-02b**：全部上游不可服务时健康位**保持健康**。
  这一条与"会变红"同等重要 —— 它证明职责划分真的落地了，而不只是写在文档里
- 原先这里写着"在人拍板之前不写这条测试"。拍板已完成，**两条都要写**
...
├── plan.md                            # 本文件
├── spec.md                            # 4 个用户故事 · 34 条 FR · 12 条 SC
├── research.md                        # Phase 0：R-01…R-08 + V-01…V-10（含 V-02b）
├── data-model.md                      # 三个「在线」谓词并排 + 三个实体
├── contracts/
│   ├── proxy-health.md                # 范围 A 的契约（§5 已定案）
│   └── recovery-capability.md         # 范围 B 的契约（含变红核对表）
├── quickstart.md                       # 14 个验收场景 A–M（含 D2 反向确认）
├── checklists/
...
tests/unit/
├── proxy-health-boundaries.test.mjs   # 新：静态禁 /ext/health（剥注释后扫）+ 探测位置不 proxy_pass
...
| **两处规格措辞问题提出交人拍板而非自行改小 FR** | 原 FR-002 后半句与 FR-003 在同一个探测里结构性冲突；FR-017 字面不可实现 | 「按自己的理解实现，事后说明」更快，但那就是悄悄改小验收标准。**而且这次拍板牵出了四处连带矛盾**（原 FR-008 / SC-007 / US1 场景 4 / 一条边界情形都要求"全部 L1 不可服务时代理健康位变红"）—— 自行实现的话那四处会一直留在验收标准里，最后得到一份自相矛盾的"已达成" |

---

## Phase 结果

- **Phase 0（研究）**：[research.md](./research.md) —— R-01…R-08 决策 + 被否方案 + V-01…V-10（含 V-02b）待实测项。
  **零个 NEEDS CLARIFICATION**；两个规格措辞问题（R-03、R-04）**已于 2026-09-11 拍板并落地**。
...
- **Phase 1 后重查宪法**：第十四条（ADR）与第十五条（逐项核验）已列为必办项，
  ...
  全部在 Phase 0 就已识别。
- **拍板后的规格状态**：**34 条 FR**（新增 FR-034）、**12 条 SC**，
  八处改动详见 [spec.md](./spec.md) 头部的修正清单。
- **`/speckit-analyze` 的结果（2026-09-11）**：0 CRITICAL、2 HIGH、4 MEDIUM、3 LOW；
  占位符 0、模糊形容词 0、重复需求 0、幽灵编号引用 0。
  已修 **C1**（FR-019 无判据 → T046/T047）、**A1**（第八条的断言为假 → 修补后成立）、
  **C2**（FR-030 无判据 → T044 加机械 diff）、**I1**（快照字段名只活在 tasks 里 → 写进 data-model）、
  **I2**（SC-001 要五台而任务写三台 → 改为五台）。
  **L1**（24/34 条 FR 在 tasks 里缺编号引用，语义已覆盖）按决定留到 T001 建 dod 时补齐。
  **D1 / D2 / D3** 三处 LOW（D2 场景在跨机形态造不出、T002 的"七个容器"跨不了机、
  Phase 5 的依赖声明偏严）**尚未处理**。

> **A1 值得单独记一句。** 那一行是我自己在 plan 阶段写下的「每条 FR 都有对应判据」，
> 而它当时就是假的 —— 34 条里有 2 条一个判据都没有。
> **一句自我声明的合规，在没有机械核验之前不算证据** ——
> 这与本期要修的那个缺陷同一形状：声明"L1 仍可服务"，而入口层面并不成立。
