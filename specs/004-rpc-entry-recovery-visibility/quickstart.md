...
| E · 两个 Primary 全停时入口可用 | SC-001 / V-03 | **5**（造场景只需 3 台，但判据要每台都发） | ⬜ 待做 |
...
## E · 两个 Primary 全停时入口可用（**需五台机器**）

两个 Primary 分处 ubuntu-1 与 ubuntu-2，单台机器造不出这个场景 ——
**造场景**只需三台，但 SC-001 的判据是**每一台**都要经本机入口发交易，
所以要五台。「入口可用」是**逐边界**的性质，不是全局的：
一台机器的代理好着，不能推出另一台的也好着（003 在 win-2 上连撞四次已经证明过这件事）。
