...
| 场景 | 内容 | 结果 |
|---|---|---|
| **A** 一键启动 | 重置后 `devnet-start` 至 READY | ✅ **80 秒** |
...
| **D** 重置与状态保留 | reset→start ×10 创世哈希全等；stop→start 高度延续 | ⏳ 见下方 SC-003 |
...
| SC-003 | 连续 10 次重置→启动创世哈希完全一致 | configVersion 1.0.0 下曾实测 10/10（启动 75–77 秒）；**1.1.0 新创世下复测进行中** | ⏳ |
