...
| 状态 | 判据 | 运维含义 |
|---|---|---|
| `stopped` | 容器**在本机存在且未运行** | 本机上被主动停止（devnet-stop / docker stop），或该节点自己退出。**不是**边界缺席 —— 那是 `unreachable` |
| `starting` | 进程在跑，API 未响应 | 等待 |
| `bootstrapping` | `info.isBootstrapped = false` | 等待 |
| `catching-up` | 已引导、在服务 L1、本地高度 < 网络高度。**不要求"在增长"**：本链无交易不出块，空闲时高度本就不动，采样窗口内没涨是常态；"是否卡住"交给持有超时窗口的容器健康检查 | **等待**——须给出进度（FR-032） |
| `healthy` | 已引导、高度追平、peers 达标 | 正常 |
| `unreachable` | 探测不到该节点，且**本机没有它的容器事实**。**有两种含义，离线语义相反**：① 其他可达节点的 peer 列表里**有**它 → 网络里它还在，是**本机到它的路径**问题，**不计入离线**；② 同边界全部节点都不应答 → **整域缺席**，计入离线 | ① 修本机的网络路径（链是好的，别去动那台机器）；② 去看那台机器。**混淆两者会虚报余量不足** |
| `identity-mismatch` | NodeID 与制品中的 `bootstrapValidators` 不符 | 终态，须处置（FR-017） |
| `data-corrupt` | 数据库打不开 | 终态，重建该节点即可（FR-006），**不需全链重置** |
| `stalled` | **未在服务 L1** 且超过窗口，**且其余验证者全部在场**。后一个条件是必需的：跨机分批启动时先起来的机器无法服务 L1，成因在别的机器未启动（未达 α/k=75% 查询门槛），本机无可处置之处 | 须处置，**且处置对象确实是本节点** |

`catching-up` 与 `stalled` 的分界是本状态机唯一需要**时间窗口**的判定。窗口长度由 SC-004（单节点追平 ≤ 2 分钟）反推，具体取值在实现期由 V-01 实测确定，不在此预设。

> **实现落点（2026-09-09）**：`docker/node/healthcheck.sh` 的 `STALL_AFTER=300`（秒），
> 依据是实测「空卷重新同步约 6s、崩溃后恢复 12–17s」，留一个量级的余量；
> 容器首次启动由 compose 的 `start_period: 120s` 兜住。判据的第二个条件（其余验证者在场）
> 与该窗口无关，见 `contracts/node-runtime.md` 的说明。
