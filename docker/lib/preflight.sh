...
# 需要的工具因镜像而异：devnet 运行时要 socat/ss/bc 做代理与端口检查，
# 而 docker/bootstrap 只建链、不承载运行，那些工具刻意没装（功能 002）。
# 调用方可用 KARMACHAIN_REQUIRED_TOOLS 覆盖清单。
: "${KARMACHAIN_REQUIRED_TOOLS:=jq curl socat ss bc avalanche}"

preflight_tools() {
  local t missing=()
  for t in ${KARMACHAIN_REQUIRED_TOOLS}; do command -v "$t" >/dev/null 2>&1 || missing+=("$t"); done
  [ ${#missing[@]} -eq 0 ] || pf_fail ${EXIT_DEPS} configuration "missing tools in image: ${missing[*]} — rebuild the image"
}
