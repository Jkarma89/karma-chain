...
switch ($Action) {
    ...
    'status'  {
        ...
        $health = docker inspect --format '{{if .State.Health}}{{.State.Health.Status}}{{else}}-{{end}}' $container 2>$null
        '{0,-12} {1,-10} {2}' -f $Node, $state, $health
    }
    'wipe'    {
        docker compose -f $ctx.Compose stop $Node 2>$null | Out-Null
        docker compose -f $ctx.Compose rm -f $Node 2>$null | Out-Null
