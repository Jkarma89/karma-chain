...
import { identityFromKeyDir, identityOf, vmIdFromChainName } from '../verify/lib/identity.mjs';
...
export function renderNodeIdentities(p = loadProtocol(), identity = readJson(IDENTITY_PATH), deploymentName) {
  ...
  const out = {};
  // 按 **keyDir** 索引，不按 validatorIndex —— `deriveTopology()` 的输出里
  // **没有 validatorIndex** 这个键（只有 id/role/httpPort/stakingPort/keyDir/domain/address/…）。
  // 第一版按 validatorIndex 查，于是永远查不到、一路回落到「从密钥派生」，
  // 对创世后加入的成员就是 ENOENT。而那次失败**在 npm test 里是看不见的**：
  // describe 体求值时抛异常，node:test 报 not ok 却不计入 fail、退出码仍为 0。
  // 现在由 tools/test/run-tests.mjs 拦住这种静默跳过。
  const byKeyDir = new Map((p.validators.nodes ?? []).map((v) => [v.keyDir, v]));
  for (const n of d.topologyNodes) {
    // 验证者的身份来源有两条（见 identityOf 的注释）：创世成员从本地密钥派生，
    // 创世后加入的凭声明的公开材料 —— 后者仓库里没有它的私钥，派生不出来。
    // Primary 节点仍走 keyDir 派生（它们在创世里）。
    const declared = n.role === 'l1-validator' ? byKeyDir.get(n.keyDir) : null;
    const id = declared ? identityOf(declared) : identityFromKeyDir(n.keyDir);
    // 三个哈希：能读到文件就现算（创世成员），否则用声明报上来的指纹。
    // **两者必须给出同一组字段名** —— 容器的 check_key_material() 逐个 jq 取值，
    // 少一个就取到 null、直接退出 12。
    const hashes = id.derived === false
      ? { certSha256: id.certSha256, keySha256: id.keySha256, signerSha256: id.signerSha256 }
      : {
